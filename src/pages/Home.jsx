...
const Home = () => {
  return (
    <div className="w-full">
      {/* Hero Section */}
      <section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden border-b border-primary/30">
        {/* Background Image */}
        <div 
        ...
        {/* Content */}
        <div className="relative z-30 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center mt-20">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
            className="inline-block mb-6 px-4 py-1 pixel-border bg-primary/10 text-primary font-mono text-xs tracking-widest"
          >
            [ WELCOME TO THE GRID ]
          </motion.div>
          ...
          <motion.p 
            ...
            className="text-lg md:text-xl text-textMuted max-w-2xl mx-auto mb-12 font-mono"
          >
          ...
          >
            <Link to="/events" className="btn-pixel w-full sm:w-auto">
              EXPLORE EVENTS →
            </Link>
          </motion.div>
        </div>
      ...
      <section className="py-24 relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-20">
            <h2 className="text-4xl md:text-5xl font-bold mb-6">BUILD SOMETHING<br/><span className="text-primary">THAT MATTERS.</span></h2>
            <p className="text-textMuted max-w-2xl mx-auto font-mono">Coder's Forum is more than just a club. It's a platform for creators, thinkers, and builders to bring their ideas to life.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <motion.div 
                ...
                className="terminal-panel p-6 pixel-border group hover:border-secondary transition-colors"
              >
                <div className="font-mono text-primary text-xl mb-4 group-hover:text-secondary transition-colors">// {pillar.id}</div>
                <h3 className="text-xl font-bold mb-3">{pillar.title}</h3>
                <p className="text-textMuted text-sm font-mono">{pillar.desc}</p>
                <pillar.icon className="absolute bottom-6 right-6 w-12 h-12 text-primary/10 group-hover:text-secondary/20 transition-colors" />
              </motion.div>
            ))}
          </div>
        </div>
